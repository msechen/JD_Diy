...
@jdbot.on(events.NewMessage(from_users=chat_id, pattern=r'(szd )'))
async def myaddszd(event):
    try:
        SENDER = event.sender_id
        messages = event.raw_text.split("\n")
        for message in messages:
            if "szd " not in message:
                continue
            
            kname = "jd_Share_Code"
            vname = message.replace("szd ", "")
            ...
            RunCommound="/ql/scripts/ModScript/ccwav_jd_share.js"
            await jdbot.send_message(chat_id, '开始执行ccwav_jd_share.js，请稍后...')
...
if ch_name:
    jdbot.add_event_handler(myaddszd, events.NewMessage(from_users=chat_id, pattern=BOT_SET['命令别名']['cron']))