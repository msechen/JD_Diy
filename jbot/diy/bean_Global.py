...
async def bot_bean(event):
    ...
    if V4 and text == 'in':
        ...
        await event.delete()
        await client.send_message(event.chat_id, '您的近日收入情况', file=BEAN_IMG)
        ...
        await event.delete()
        await client.send_message(event.chat_id, '您的近日支出情况', file=BEAN_IMG)
        
    elif not V4 and (text == 'in' or text == 'out' or text is None):
        await event.delete()
        await client.send_message(event.chat_id,'QL暂不支持使用bean in、out ,请使用/bean n n为数字')
        
    elif text and int(text):
        res = get_bean_data(int(text))
        if res['code'] != 200:
            await event.delete()
            await client.send_message(event.chat_id, f'something wrong,I\'m sorry\n{str(res["data"])}')
        else:
            creat_bean_count(res['data'][3], res['data'][0], res['data'][1], res['data'][2][1:])
            await event.delete()
            await client.send_message(event.chat_id, f'您的账号{text}收支情况', file=BEAN_IMG)
    elif not text:
        subprocess.check_output(
            'jcsv', shell=True, stderr=subprocess.STDOUT)
        creat_bean_counts(BEAN_TOTAL_FILE)
        await event.delete()
        msg=await client.send_message(event.chat_id, '您的总京豆情况', file=BEAN_IMG)
        await asyncio.sleep(5)
        await client.delete_messages(event.chat_id, msg)
    else:
        await event.delete()
        await client.send_message(event.chat_id, '青龙暂仅支持/bean n n为账号数字') 
