...
@jdbot.on(events.NewMessage(from_users=chat_id, pattern=r'(lk )'))
async def myaddlk(event):
    try:
        SENDER = event.sender_id
        messages = event.raw_text.split("\n")
        for message in messages:
            if "lk " not in message:
                continue
            
            kname = "JD_LUCK_DRAW_URL"
            vname = message.replace("lk ", "")
            ...
            RunCommound="/ql/scripts/ModScript/m_jd_wx_luckDraw_ccwavMod.js"
            await jdbot.send_message(chat_id, '开始执行m_jd_wx_luckDraw_ccwavMod.js，请稍后...')
...
if ch_name:
    jdbot.add_event_handler(myaddlk, events.NewMessage(from_users=chat_id, pattern=BOT_SET['命令别名']['cron']))